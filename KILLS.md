# ~~Kills ledger — append-only~~ — now the ledger of sittings
*(SUPERSEDED, ruled by counsel on David's instruction, 2026-09-09 — council
entry 7: this document records kills, stops AND passes, not kills alone.
"Append-only" still means never delete a record — it does not mean oldest-
last: the hosted era below runs **newest first, keyed on the MERGE
INSTANT** — and, where a board reached main by a single-parent commit
rather than a merge, on that commit's own instant — never the printed
date, which is local in some rows and UTC in others (council entry 33).
The pre-host section and the 2026-08-25 seed pair are not reordered by
this supersession. Every PASSED record names the board's path in
backticks exactly as `public/index.html` hrefs it, next to the **PASSED**
marker, because that path — never the prose board name — is the key a
hub-to-ledger check reads. Cites out of this file into documents that
move name a section heading or a dated record rather than a line number,
per the de-lining rule in `docs/sweep-2026-09-02.md` (council entry 33).
The bound named below is never the word "every" standing alone: it is
every board `public/index.html` tags **live — passed its sit** at the
landing HEAD, checkable against that file, plus every kill and stop
already recorded. No hosted-era row, kill or pass, can meet the build-sha
line below — `public/build-info.json`'s `commit` field still reads
`02e026b`, `builtAt` 2026-08-24T16:30:48Z, though the file itself last
moved at `ae96ad9` — so hosted-era rows substitute the landing commit
plus the index and sim content sha256s instead; the pre-host section
below, which carries no shas at all, is the sha-less exception this
predates.)*

Every kill records: date, board, build sha (from /build-info.json at the
time of the sit), what the player did, the recut decision. A kill without
a sha is a story, not a kill. Supersession is an edit with a forward
...
- 2026-09-04 · Still Standing · `/still-standing/` · **PASSED** · commit
  `685bf3f` / index `69ab4662` / sim `6b3b9267`. David's sit, verbatim: "I
  was able to do runs to mosswake over adn over until then one failed and
  the larder covered it and then I kept going until there was a nother
  issue and the larder could not cover it twice at which poiint the
  instance was over and I could not do anything else." Verdict, verbatim:
  "Pass, and land it against outcome 2 as registered." Typos carried (adn,
  a nother, poiint), per the ledger convention this file already keeps —
  `685bf3f`'s own body records catching a tidy of "a nother" INSIDE the
  quotation marks and correcting it. Sit recorded in commit body
  `685bf3f`. Record: Linear CFD-212.

- 2026-09-02 · Two Ways from Here · `/two-ways-from-here/` · **PASSED** ·
  commit `c1c41df` / index `d800c8de` / sim `13bb2d43`. David's sit,
  verbatim: "a run came home short and hte larder was able to cover the
  difference. It made sense what was ahppening and clicking around to the
  different buildings and options is feeling more like a full expeirnece
  with context." Asked how it ended: "I banked it - clicked Collect and it
  ended there." Verdict, verbatim: "pass - move forward." Typos carried
  (hte, ahppening, expeirnece), per the ledger convention. Sit recorded in
  commit body `c1c41df`. Record: Linear CFD-210.

- 2026-09-01 · Dice at the Places · `/dice-at-the-places/` · **PASSED** ·
  merge `cbf537c` (PR #21, signed 2026-08-31) / pinned `12ccd3d` / index
  `d97d9951` / sim `f64b4309`. David's sit, verbatim: "I could send
  Mosswake runs, which I did, then the run came home short and the larder
  covered it." Sit recorded in `docs/cfd-209-beat.md`'s "## PASSED —
  David, 2026-09-01. (Signed 2026-08-31.)" section; same verbatim in commit
  body `12ccd3d`. Record: Linear CFD-209.
...
- 2026-08-31 · They Remember · cut 0 · merge `7bcc315` (PR #19, 2026-08-31
  ...
  PASSED at `a525218`, 2026-08-31 — `/they-remember/` · index `acbf4304`
  **unmoved**, sim `a3345903`, so the index hash does not distinguish the
  kill from the pass.
  Sit recorded at `docs/cfd-208-beat.md:346-350`; same verbatim in commit body
  `3c22c9a`. Record: Linear CFD-208.

- 2026-08-30/31 · Herbs in the Larder · `/herbs-larder/` · **PASSED** ·
  merge `6e606e5` (PR #18) / index `676587bc` / sim `76c886b9`. David's
  sit: (verbatim on Linear CFD-207; not in repo — the repository holds only
  the label "Herbs placed", in `docs/cfd-208-beat.md`'s account of what cut
  0 put in front of the tap). Date bounded, not a sit time: merge
  2026-08-31 00:40:54Z, first repo record `b05502d` at 01:57:51Z, 77
  minutes later, and no pass date is written down between. The printed date
  spans two days because the two zones a reader may use straddle midnight,
  not because the bound is uncertain: printed 2026-08-30/31 per counsel's
  ruling on ledger entry 7, action 3(ii); both bounding instants are
  2026-08-31 UTC (2026-08-30 in -04:00). Record: Linear
  CFD-207.

- 2026-08-30/31 · Mosswake Loop · `/mosswake-loop/` · **PASSED** · merge
  `510a392` (PR #17) / index `6c30179c` / sim `f5407bca`. David's sit:
  (verbatim on Linear CFD-206; not in repo — the pass survives only as a
  do-not-recut pin in the next beat's Seat, which carries the same merge
  and both hashes: "Do not recut `/mosswake-loop/` (PASSED sit; live merge
  `510a392`, index `6c30179c`, sim `f5407bca`)", `docs/cfd-207-beat.md`,
  first recorded at commit `93eaf3e`). Date bounded by the merge,
  2026-08-30 23:50:43Z, and that first record, 2026-08-31 00:23:30Z, 33
  minutes later — those two genuinely cross midnight UTC; no pass date is
  written down between them. Record: Linear CFD-206.

- 2026-08-30 · Dawnspur Halt · the walk · `/dawnspur-halt/` · **PASSED** ·
  merge `66b5507` (PR #16) / index `b5a56a14` / sim `6eb957e7`. David's
  sit, verbatim: "Lit the lantern and tended home and provided resources."
  — canon §7.4's account of the recut that passed. (§7.4 is prose at HEAD
  and holds no table; the "evidence table" phrasing used by the Dawnspur Site row
  below is inherited and does not match the section as it now reads.) No
  dated pass record exists in `docs/cfd-205-halt-beat.md`; the earliest
  repo mention is commit `f797300`, 2026-08-30 23:40:32Z, 2h24m after the
  merge — both instants fall on 2026-08-30 UTC. Record: Linear CFD-205.
...
- 2026-08-28 · Dawnspur — the join · `/dawnspur-line/` · **PASSED** ·
  commit `7f58c8f` / index `b6f21db0` / sim `18b1324f`. David, 2026-08-28,
  in full: "passes". Punctuation as in the source: the period sits OUTSIDE
  the quotation, and `7f58c8f`'s body reads `David, 2026-08-28, in full:
  "passes". /dawnspur-line/ is a passed board.` Do not tidy it inward. Sat
  2026-08-27, recorded that day as played-to-conclusion with the verdict
  explicitly outstanding; the verdict landed the next day, separately — a
  completed sitting is not a verdict. Record: Linear CFD-203.

- 2026-08-26 · Dawnspur — the dispatch sitting · `/dawnspur-dispatch/` ·
  **PASSED**, fourth sitting · host `663d4fa` / index `31aead60` / sim
  `576ce2b6`. David's sit, verbatim: "I think this worked well overall."
  The record it ended on, verbatim from the board: "The first Chartered
  cargo is home. Ten runs out, seven cargoes banked, three turned back, 4
  marks staked and lost on the way." Sit recorded in
  `docs/cfd-196-beat.md`'s "# The sit — PASSED, David, 2026-08-26"
  section. Record: Linear CFD-196.
...
- 2026-08-25 · Dawnspur scale sitting · build `5fabe7c` / index `d6d5e262` / sim
  ...
  The recut passed: **PASSED** · `/dawnspur-scale/` · commit `1aea5402` /
  index `5d2f452f` / sim `953368a1`. No David verbatim for the recut's own
  sit is recorded in this repository; Linear CFD-183 holds it. Hub: "live
  — passed its sit 2026-08-25".
...
- 2026-08-24 · Dawnspur · starve-or-feed first cut · index `bdde9b50` / sim
  ...
  That sibling passed: **PASSED** · `/dawnspur-heat/` · index `b5f7e14f` /
  sim `292d6645`, date null. The lower bound is the commit that put those
  bytes on main, `efbed23`, 2026-08-25 20:17:50Z — NOT the `4c5e9ec`
  commit at 16:25:22Z, which brought only the base cut: measured this
  session, `4c5e9ec`'s index is `b6e815c0` and `b5f7e14f` first exists at
  `efbed23` (sim `292d6645` does match at `4c5e9ec`). The chain is
  `4c5e9ec` (base) + `db1a9ad` + `c887359` / `efbed23`, per
  `docs/cfd-176-beat.md`'s Seat paragraph, which is also where the pass's
  own emptiness is recorded, verbatim: "The pass has no `KILLS.md` line and
  no verbatim in this repository; its record is Linear CFD-176, date null."
